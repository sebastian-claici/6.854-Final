...
The majority of the time is spent in invocations of \textsc{Process}. We say that an invocation of \textsc{Process} on region $R$ is \emph{truncated} if $Q(R).minKey()$ is infinity at the end of the invocation. All level $0$ invocations are truncated. The crux of the analysis relies on a \emph{charging invariant}:
...
\begin{tabular}{@{}lllll@{}} \toprule
  Level & Calls & Time per invocation & No.\ $(R,v)$ pairs & Charge\\ \midrule
  $2$ & $1$ & $O(\log n)$ & 1 & 1 + 1 + $\log n$\\
  $1$ & $\log n$ & $O(\log n \log \log n)$ & $O(n/\log^2 n)$ & $1 + \log n$\\
  $0$ & $0$ & $O(1)$ & $O(n)$ & $1$\\
  \bottomrule
\end{tabular}
\end{table*}
